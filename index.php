...
function text_content_limit($text, $limit = 300) {
    ...
    if (strlen($text) <= $limit) {
        ...
        $preview_content = substr($text,0,$limit);

        $stop_index = strrpos($preview_content, ' ');

        $remaining_content = substr($text,$stop_index); // если вдруг понадобится оставшаяся часть

        return "<p>". substr($preview_content,0, $stop_index) . "..." .
                    "<br>" .
